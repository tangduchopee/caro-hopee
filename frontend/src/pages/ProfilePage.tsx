    ...
    return (
      <Container maxWidth="md" sx={{ py: 8 }}>
        <Box sx={{ textAlign: 'center' }}>
          <Typography variant="h6" sx={{ color: '#5a6a7a' }}>{t('profile.loginRequired')}</Typography>
        </Box>
      </Container>
    );
  }
            ...
            <Box>
              ...
              {currentGameStats.lastPlayed && (
                <Typography variant="caption" sx={{ color: '#8a9ba8', mt: 2, display: 'block' }}>
                  {t('profile.lastPlayed')}: {new Date(currentGameStats.lastPlayed).toLocaleDateString(language === 'vi' ? 'vi-VN' : 'en-US')}
                </Typography>
              )}
            </Box>
          ) : (
